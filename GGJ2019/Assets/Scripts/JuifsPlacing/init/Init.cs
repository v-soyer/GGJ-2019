        ...
        if (all.juifs[i].select == false)
        {
            for (int nb = 0; nb < all.juifs.Count; nb++)
            {
                if (all.juifs[nb].select == true)
                {
                    ...
                    all.juifs[nb].button.GetComponent<RectTransform>().sizeDelta = new Vector2(160.0f, 73.79f);
                }
            }
            all.juifs[i].select = true;
            all.juifs[i].button.image.sprite = all.juifs[i].onSelect;
            all.juifs[i].button.GetComponent<RectTransform>().sizeDelta = new Vector2(185.0f, 73.79f);
            
        } else
        {

            all.juifs[i].button.GetComponent<RectTransform>().sizeDelta = new Vector2(160.0f, 73.79f);
